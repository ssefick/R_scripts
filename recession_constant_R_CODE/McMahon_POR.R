...
#this incorrectly removes one site with a reading (didn't stop
#when removed from the water). Specifically wpms @ ftbr
x. <- remove.0.zoo(x)

#x. <- na.approx(x.)

#start <- contiguous.zoo(x.)$start

#end <- contiguous.zoo(x.)$end
...
#############rise flashiness####################
periodr.3.048 <- function(x){sum(na.omit(x)>=3.048)}
periodr.9.144 <- function(x){sum(na.omit(x)>=9.144)}
periodr.15.24 <- function(x){sum(na.omit(x)>=15.24)}
periodr.21.336 <- function(x){sum(na.omit(x)>=21.336)}
periodr.27.432 <- function(x){sum(na.omit(x)>=27.432)}
...
###############fall flashiness###################
periodf.3.048 <- function(x){sum(na.omit(x)<=-3.048)}
periodf.9.144 <- function(x){sum(na.omit(x)<=-9.144)}
periodf.15.24 <- function(x){sum(na.omit(x)<=-15.24)}
periodf.21.336 <- function(x){sum(na.omit(x)<=-21.336)}
periodf.27.432 <- function(x){sum(na.omit(x)<=-27.432)}
...
###################################################
##################quantile functions###############
q75 <- function(x){ifelse(x=="NA", NA, ifelse(x>quantile(x, 0.75, na.rm=T), x, NA))}
q90 <- function(x){ifelse(x=="NA", NA, ifelse(x>quantile(x, 0.90, na.rm=T), x, NA))}
q95 <- function(x){ifelse(x=="NA", NA, ifelse(x>quantile(x, 0.95, na.rm=T), x, NA))}
q25 <- function(x){ifelse(x=="NA", NA, ifelse(x<quantile(x, 0.25, na.rm=T), x, NA))}
q10 <- function(x){ifelse(x=="NA", NA, ifelse(x<quantile(x, 0.10, na.rm=T), x, NA))}
q5 <- function(x){ifelse(x=="NA", NA, ifelse(x<quantile(x, 0.05, na.rm=T), x, NA))}
###################################################
...
CV. <- data.frame(apply(y, 2, function(x)CV(x, na.rm=T)))
...
hydrology$months <- length(b[,1])/(24*30)

out.name <- paste(name, ".pdf", sep="")
